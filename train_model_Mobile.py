...
import logging
from mobile_model import MobileNetV3SmallUNet
...
def find_latest_checkpoint(checkpoint_dir):
    ...
    pattern = re.compile(r"mobile_small_epoch_(\d+)\.pth")
    max_epoch = 0
...
def get_dataloader(dataset_name, transform, subset_size, skipped_images):
    if dataset_name == 'LoL':
        dataset = LoLDataset(
            base_path='/UHD-Low-light-image-enhancement/database/database_lol/our485',
            transform=transform,
...
def main(num_epochs=50, subset_size=None, dataset_name='LoL'):
    ...
    model = MobileNetV3SmallUNet().to(device)  # Ensure you're using the correct model class
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f'mobile_small_epoch_{epoch + 1}.pth')
        torch.save({
            'epoch': epoch + 1,
...
if __name__ == '__main__':
    main(num_epochs=100,subset_size=None, dataset_name='LoL')  # Adapt parameters
